...
from modules.mms import asmr, asmr_res, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, generator_loss, \
    discriminator_loss, asmr_full
...
def train(rank, cfg, a):
    ...
    # Model DDP
    generator = asmr_full(cfg.embed, cfg.hifigan, cfg.mamba).to(device)
    mpd = MultiPeriodDiscriminator().to(device)
    msd = MultiScaleDiscriminator().to(device)
    generator = DistributedDataParallel(module = generator, device_ids = [rank]).to(device) # find_unused_parameters=True when debugging
    mpd = DistributedDataParallel(module=mpd, device_ids=[rank]).to(device)
    msd = DistributedDataParallel(module=msd, device_ids=[rank]).to(device)
    ...
    for epoch in range(max(0, last_epoch), cfg.train["n_epoch"]):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss_mel = F.l1_loss(y_mel, y_g_hat_mel) * 45 # Default to 45
            loss_abs = F.l1_loss(y_high, y_low) * 10
            ...
            loss_gen_all = loss_abs + loss_gen_s + loss_gen_f + loss_fm_s + loss_fm_f + loss_mel
            ...
            if rank == 0:
                ...
                # Validation
                if steps % cfg.train["validation_interval"] == 0 and steps != 0:  # and steps != 0:
                    generator.eval()
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            loss_mel = F.l1_loss(y_mel, y_g_hat_mel) * 40  # Default to 45
                        ...
                        wandb.log({"validation/generation_error": val_err, "steps": steps})
                        ...
                        if steps % cfg.train["checkpoint_interval"] == 0 and steps != 0:
                            ...
                            best_checkpoint_path = "{}/g_{:08d}".format(cfg.data["ckpt_save_path"], best_step)
...
def main():
    ...
    parser.add_argument('--experiment_name', default='asmr_full_with_l1')
    a = parser.parse_args()
    ...
    world_size = 8
    mp.spawn(train, args=(hyperparameters, a), nprocs=4)
